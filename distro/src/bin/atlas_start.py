...
METADATA_DEBUG = " -Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=8000 -server -Xdebug -Xnoagent "
# debug on/off
# METADATA_LOG_OPTS=METADATA_DEBUG+"-Datlas.log.dir=%s -Datlas.log.file=application.log"
    ...
    if os.path.isfile(metadata_pid_file):
        ...
        if mc.ON_POSIX:
            ...
            if mc.IS_WINDOWS:
                ...
                # os other than nt or posix - not supported
                # need to delete the file to restart server if pid no longer exist
                mc.server_already_running(pid)
