...
angular.module('dgc.lineage').controller('Lineage_commonController', ['$element', '$scope', '$state', '$stateParams', 'lodash', 'LineageResource', 'd3', 'DetailsResource', '$q',
    function($element, $scope, $state, $stateParams, _, LineageResource, d3, DetailsResource, $q) {
        ...
        function getCombinedLineageData(entityData, callRender) {
            var queryInterf;
            var typeIn;
            var typeOut;

            if ($scope.isDataContainer) {
                typeIn = 'inputs';
                typeOut = 'outputs';
                queryInterf = LineageResource.queryDataContainerIO;
            } else if ($scope.isDataTable) {
                typeIn = 'inputs';
                typeOut = 'outputs';
                queryInterf = LineageResource.queryDataTableIO;
            } else if ($scope.isHiveTable) {
                typeIn = 'inputs';
                typeOut = 'outputs';
                queryInterf = LineageResource.queryHiveIO;
            } else if ($scope.isDataField) {
                typeIn = 'source';
                typeOut = 'target';
                queryInterf = LineageResource.queryDataFieldST;
            } else if ($scope.isStep) {
                typeIn = 'succeeding';
                typeOut = 'preceding';
                entityData.queryType = 'step';

                queryInterf = LineageResource.queryStepPS;
            } else if ($scope.isAction) {
                typeIn = 'succeeding';
                typeOut = 'preceding';
                entityData.queryType = 'action';

                queryInterf = LineageResource.queryStepPS;
            } else if ($scope.isTask) {
                typeIn = 'succeeding';
                typeOut = 'preceding';
                entityData.queryType = 'task';
            ...
            queryInterf({
                qualifiedName: entityData.qualifiedName,
                queryType: entityData.queryType,
                type: typeOut
        ...
        $scope.queryType = $element.parent().attr('lineage-scope-type');
        ...
        function transformData(metaData) {
            ...
            function getLoadProcessTypes(guid) {
                ...
                angular.forEach(guidsList, function(value) {
                    if (value.id.id === guid) {
                        var processResName;
                        if ($scope.isStep || $scope.isAction || $scope.isTask) {
                            processResName = value.values.kind;
                        } else if ($scope.isDataField) {
                            processResName = value.values.dependencyInfo.values.description;
                        } else {
                            processResName = value.values.name;
                        }

                        if (typeof processResName !== "undefined"  && (null !== processResName)) {
                            procesRes.name = processResName;
                        } else {
        ...
        function getVertexImg(nodeType) {
            ...
            } else if ($scope.isDataTable || $scope.isHiveTable) {
