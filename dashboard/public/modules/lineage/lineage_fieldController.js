...
angular.module('dgc.lineage').controller('Lineage_fieldController', ['$element', '$scope', '$state', '$stateParams', 'lodash', 'LineageResource', 'd3', 'DetailsResource', '$q',
    function($element, $scope, $state, $stateParams, _, LineageResource, d3, DetailsResource, $q) {
        ...
        function getCombinedLineageData(fieldData, callRender) {
            LineageResource.queryDataFieldST({
                qualifiedName: fieldData.qualifiedName,
                queryType: fieldData.queryType,
                type: 'target'
        ...
        $scope.type = $element.parent().attr('data-field-type');
        $scope.queryType = $element.parent().attr('lineage-scope-type');
        $scope.qualifiedName = $scope.$parent.qualifiedName;
        ...
        function onActive(lineageData) {
            console.log($scope.qualifiedName);
        ...
        function transformData(metaData) {
            ...
            function getLoadProcessTypes(guid) {
                ...
                angular.forEach(guidsList, function(value) {
                    if (value.id.id === guid) {
                        var description = value.values.dependencyInfo.values.description;
                        if (typeof description !== "undefined"  && (null != description)) {
                            procesRes.name = description;
                        } else {
                            procesRes.name = 'LineageProcess';
                        }
                        procesRes.typeName = 'LineageProcess';
                        procesRes.tip = value.values.dependencyInfo.values.function;
                    }
                });
                return procesRes;
            }
            ...
            function update(source) {
                ...
                nodeEnter.append("image")
                    ...
                    .attr("xlink:href", function(d) {
                        return d.type === 'DataField' ? '../img/field.png' : '../img/process.png';
                    })
                    .on('mouseover', function(d) {
                        if (d.type === 'LineageProcess' || 'DataField') {
                            tooltip.show(d);
                        }
                    })
                ...
                node.select("image.nodeImage")
                    ...
                    .attr("xlink:href", function(d) {
                        if (d._children) {
                            return d.type === 'DataField' ? '../img/field1.png' : '../img/process1.png';
                        }
                        return d.type === 'DataField' ? '../img/field.png' : '../img/process.png';
                    });
