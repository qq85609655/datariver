...
angular.module('dgc.lineage').controller('Lineage_actionController', ['$element', '$scope', '$state', '$stateParams', 'lodash', 'LineageResource', 'd3', 'DetailsResource', '$q',
    function($element, $scope, $state, $stateParams, _, LineageResource, d3, DetailsResource, $q) {
        ...
        $scope.type = $element.parent().attr('data-sequence-type');
        ...
        function transformData(metaData) {
            ...
            function getLoadProcessTypes(guid) {
                ...
                angular.forEach(guidsList, function(value) {
                    if (value.id.id === guid) {
                        if (typeof value.values.kind !== "undefined" && (null != value.values.kind)) {
                            procesRes.name = value.values.kind;
                        } else {
                            procesRes.name = 'StepSequence';
                        }
                        procesRes.typeName = 'StepSequence';
                    }
                });
                return procesRes;
            }
            ...
            function update(source) {
                ...
                nodeEnter.append("image")
                    ...
                    .attr("xlink:href", function(d) {
                        return d.type === 'step' ? '../img/action.png' : '../img/process.png';
                    })
                ...
                node.select("image.nodeImage")
                    ...
                    .attr("xlink:href", function(d) {
                        if (d._children) {
                            return d.type === 'step' ? '../img/action1.png' : '../img/process1.png';
                        }
                        return d.type === 'step' ? '../img/action.png' : '../img/process.png';
                    });
