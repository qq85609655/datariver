...
angular.module('datamap.statistic').controller('LineageController', ['$element', '$scope', '$state', '$stateParams', 'lodash', 'LineageResource', 'd3', 'DetailsResource', '$q',
    function($element, $scope, $state, $stateParams, _, LineageResource, d3, DetailsResource, $q) {
        ...
        function getQueryInterf(entityType) {
            if ('DataContainer' === entityType) {
                return LineageResource.queryDataContainerIO;
            } else if ('DataTable' === entityType) {
                return LineageResource.queryDataTableIO;
            } else if ('DataField' === entityType) {
                return LineageResource.queryDataFieldST;
            } else if ('ETLStep' === entityType || 'WorkflowAction' === entityType || 'ETLTask' === entityType) {
                return LineageResource.queryStepPS;
            }
        }

        function getCombinedLineageData(dbData, callRender) {
            var queryInterf = getQueryInterf(dbData.entityType);
            if (typeof queryInterf === 'undefined') {
                $scope.requested = false;
                return;
            }

            queryInterf({
                qualifiedName: dbData.qualifiedName,
        ...
        $scope.queryType = 'task';
        $scope.entityType = 'DataContainer';
        $scope.qualifiedName = 'mysql.con99.default.test2';
        ...
        $scope.onReset = function() {
            if (typeof $scope.lineageData !== "undefined") {
                renderGraph($scope.lineageData, {
                    eleObj: $element,
                    element: $element[0],
                    height: $scope.height,
        ...
        function transformData(metaData) {
            ...
            function getNode(guid) {
                var name, type, tip, nodeType;
                if (vertices.hasOwnProperty(guid)) {
                    name = vertices[guid].values.qualifiedName;
            ...
            function getLoadProcessTypes(guid) {
                ...
                angular.forEach(guidsList, function(value) {
                    if (value.id.id === guid) {
                        if (typeof value.values.name !== "undefined"  && (null !== value.values.name)) {
                            procesRes.name = value.values.name;
                        } else {
                            procesRes.name = 'LineageProcess';
                        }
                        procesRes.typeName = 'LineageProcess';
                    }
                });
                return procesRes;
            }
        ...
        function getVertexImg(nodeType) {
            if (nodeType === 'edge') {
                return '../img/process.png';
            }

            if ('DataContainer' === $scope.entityType) {
                return '../img/database.png';
            } else if ('DataTable' === $scope.entityType) {
                return '../img/tableicon.png';
            } else if ('DataField' === $scope.entityType) {
                return '../img/field.png';
            } else if ('ETLStep' === $scope.entityType) {
                return '../img/step.png';
            } else if ('WorkflowAction' === $scope.entityType) {
                return '../img/action.png';
            } else if ('ETLTask' === $scope.entityType) {
                return '../img/task.png';
            }
        }

        function getVertexImg1(nodeType) {
            if (nodeType === 'edge') {
                return '../img/process1.png';
            }

            if ('DataContainer' === $scope.entityType) {
                return '../img/database1.png';
            } else if ('DataTable' === $scope.entityType) {
                return '../img/tableicon1.png';
            } else if ('DataField' === $scope.entityType) {
                return '../img/field1.png';
            } else if ('ETLStep' === $scope.entityType) {
                return '../img/step1.png';
            } else if ('WorkflowAction' === $scope.entityType) {
                return '../img/action1.png';
            } else if ('ETLTask' === $scope.entityType) {
                return '../img/task1.png';
            }
        }
        ...
        function renderGraph(data, container) {
            ...
            var element = d3.select(container.element),
                widthg = container.width,
                heightg = container.height,
            ...
            function centerNode(source) {
                var scale = (depthwidth === 10) ? zoomListener.scale() : 0.8;
                var x = -source.y0;
                var y = -source.x0;
                x = x * scale;
            ...
            function update(source) {
                ...
                nodeEnter.append("image")
                    ...
                    .attr("xlink:href", function(d) {
                        return getVertexImg(d.nodeType);
                    })
                    .on('mouseover', function(d) {
                        if (d.type === 'LineageProcess' || 'DataContainer') {
                            tooltip.show(d);
                        }
                    })
                ...
                node.select("image.nodeImage")
                    ...
                    .attr("xlink:href", function(d) {
                        if (d._children) {
                            return getVertexImg1(d.nodeType);
                        }
                        return getVertexImg(d.nodeType);
                    });
