...
public abstract class HierarchicalType<ST extends HierarchicalType, T> extends AbstractDataType<T>
    ...
    protected Pair<FieldMapping, ImmutableMap<String, String>> constructFieldMapping(ImmutableList<String> superTypes,
        ...
        while (pathItr.hasNext()) {
            ...
            // 此处添加时间信息
            // 暂时注释掉，否则测试用例不通过
            // superTypeFields = addAttribute(superTypeFields);
    ...
    private ImmutableList<AttributeInfo> addAttribute(ImmutableList<AttributeInfo> superTypeFields) throws AtlasException {
        List<AttributeInfo> superTypeFields2 = new ArrayList<>();
        superTypeFields2.addAll(superTypeFields);
        superTypeFields2.add(new AttributeInfo(typeSystem, new AttributeDefinition("entityCreateTime", DataTypes.LONG_TYPE.getName(), Multiplicity.OPTIONAL, false,
                null), null));
        superTypeFields = ImmutableList.copyOf(superTypeFields2);
    ...
    /**
     * 比较此类型与指定类型（父类型）的大小。
     * 如果此类型是指定类型（父类型）的子类型，则返回1；
     * 如果此类型是指定类型（父类型）的父类型，则返回-1；
     * 如果不是父子关系，则判断类型的名称，如果此类型的名称小于指定类型的名称，则返回小于0；
     *                                 如果此类型的名称大于指定类型的名称，则返回大于0。
     *                                 否则返回0。
     * @param o           指定父类型
     * @return            大于0，此类型是指定类型的子类型，或者名称大于指定类型；
     *                    小于0，此类型不是指定类型的子类型，或者名称小于指定类型；
     *                    等于0，名称相同。
     */
    @Override
