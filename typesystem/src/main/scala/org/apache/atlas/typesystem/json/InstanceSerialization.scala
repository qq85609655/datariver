...
object InstanceSerialization {
  ...
  /*dtdream num:0189
  * 由于目前该类只支持json串转换为Struce或Referenceable的方法
  * 所以增加从json串转换为Id的方法，
  * 方便GraphBackedMetadataRepository类的updateEntity方法将输入的json串转换成Id
  * */
  def fromJsonId(jsonStr: String, withBigDecimals : Boolean = false): Id = {
    implicit val formats = buildFormat(withBigDecimals)
    val _s = read[_Id](jsonStr)
    asJava(_s).asInstanceOf[Id]
  }
}
