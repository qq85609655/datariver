...
/**
 * A ClosureQuery to compute '''Lineage''' for dt entities. Assumes the Lineage relation is captured in a ''CTAS''
 * type, and the entity relations are captured as attributes from a CTAS instance to entity instances.
 *
 * @param entityTypeName The name of the entity Type.
 * @param ctasTypeName The name of the Create entity As Select(CTAS) Type.
 * @param ctasPrecedingAttribute The attribute in CTAS Type that associates it to the preceding entities.
 * @param ctasSucceedingAttribute The attribute in CTAS Type that associates it to the succeeding entities.
 * @param depth depth as needed by the closure Query.
 * @param selectAttributes as needed by the closure Query.
 * @param withPath as needed by the closure Query.
 * @param persistenceStrategy as needed to evaluate the Closure Query.
 * @param g as needed to evaluate the Closure Query.
 */
case class DtLineageQuery(entityTypeName: String,
                          entityName: String,
                          ctasTypeName: String,
                          ctasPrecedingAttribute: String,
                          ctasSucceedingAttribute: String,
                          depth: Option[Int],
                          selectAttributes: Option[List[String]],
                          withPath: Boolean,
                          persistenceStrategy: GraphPersistenceStrategies,
  ...
  val closureType: String = entityTypeName

  val attributeToSelectInstance = "qualifiedName"
  val attributeTyp = DataTypes.STRING_TYPE

  val instanceValue = entityName

  lazy val closureRelation = List(
    ReverseRelation(ctasTypeName, ctasSucceedingAttribute),
    Relation(ctasPrecedingAttribute)
  )
}

/**
 * A ClosureQuery to compute where a entity is used based on the '''Lineage''' for dt entities.
 * Assumes the Lineage relation is captured in a ''CTAS''
 * type, and the entity relations are captured as attributes from a CTAS instance to entity instances.
 *
 * @param entityTypeName The name of the entity Type.
 ...
 * @param g as needed to evaluate the Closure Query.
 */
case class DtWhereUsedQuery(entityTypeName: String,
                            entityName: String,
                            ctasTypeName: String,
                            ctasPrecedingAttribute: String,
                            ctasSucceedingAttribute: String,
  ...
  lazy val closureRelation = List(
    ReverseRelation(ctasTypeName, ctasPrecedingAttribute),
    Relation(ctasSucceedingAttribute)
  )
}
