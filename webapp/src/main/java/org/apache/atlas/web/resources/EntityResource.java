...
/**
 * <p>实体管理接口。<br/>
 * 实体是类型的实例，实体定义时包含的内容和类型的定义一致。<br/>
 * <br/>
 * 包含的接口：<br/>
 * 1. 创建实体<br/>
 * curl -d {} http://localhost:21000/api/atlas/entities<br/>
 * 2. 根据实体类型或者实体的类型及属性和值来读取实体<br/>
 * curl -s 'http://localhost:21000/api/atlas/entities?type=?&property=?&value=' | python -mjson.tool<br/>
 * 3. 根据GUID来读取实体<br/>
 * curl -s http://localhost:21000/api/atlas/entities/{guid} | python -mjson.tool<br/>
 * 4. 为实体添加属性<br/>
 * curl -d {}  http://localhost:21000/api/atlas/entities/{guid}?property=?value= | python -mjson.tool<br/>
 * 5. 获取实体的trait列表<br/>
 * curl -s  http://localhost:21000/api/atlas/entities/{guid}/traits | python -mjson.tool<br/>
 * 6. 为实体添加trait<br/>
 * curl -s  http://localhost:21000/api/atlas/entities/{guid}/traits | python -mjson.tool<br/>
 * 7. 从实体上删除trait<br/>
 * curl -s  http://localhost:21000/api/atlas/entities/{guid}/traits/{traitName} | python -mjson.tool</p>
 */
@Path("entities")
@Singleton
@JsonRootType
...
public class EntityResource {
     ...
     * @param attribute  实体的属性名
     * @param value      实体的属性值
     * @return JSON格式数据，如果只输入实体类型时，则包括servlet的请求Id、实体类型、实体定义列表和列表中实体个数；如果再输入属性及属性值时，则包括servlet的请求Id、实体定义。
     */
    @GET
    ...
    /**
     * 创建实体。请求时，消息体是JSON格式的实体定义，元数据服务负责根据类型解析实体的属性，生成实体定义。
     *
     * @return JSON格式数据，包括servlet的请求Id、元数据服务创建实体的GUID和被创建实体的内容。
     */
    @POST
     ...
     * @param guid 实体的GUID值
     *
     * @return JSON格式数据，包括servlet的请求Id、输入的GUID和实体的内容。
     */
    @GET
    @Path("{guid}")
    @Produces(Servlets.JSON_MEDIA_TYPE)
    ...
    @Produces(Servlets.JSON_MEDIA_TYPE)
    @org.codehaus.enunciate.jaxrs.TypeHint(EntityResource.class)
    public Response getEntitesDefinitions(@QueryParam("guids") String guids) {
        try {
            ...
            for (String guid : guidArray) {
                if (StringUtils.isNotEmpty(guid)) {
                    final String entityDefinition = metadataService.getEntityDefinition(guid);
                    definitions.put(new JSONObject(entityDefinition));
                }
            }
            JSONObject response = new JSONObject();
            response.put(AtlasClient.REQUEST_ID, Servlets.getRequestId());
            ...
            LOG.error("Unable to get entity list for guids {}", guids, e);
            throw new WebApplicationException(Servlets.getErrorResponse(e, Response.Status.BAD_REQUEST));
        } catch (Throwable e) {
            LOG.error("Unable to get entity list for guids {}", guids, e);
            throw new WebApplicationException(Servlets.getErrorResponse(e, Response.Status.INTERNAL_SERVER_ERROR));
     ...
     *
     * @param guid 实体的GUID
     * @return JSON格式数据，包括包括servlet的请求Id、实体的GUID、给定实体的trait列表和列表大小。
     */
    @GET
    @Path("{guid}/traits")
    @Produces(Servlets.JSON_MEDIA_TYPE)
     ...
     *
     * @param guid 实体的GUID
     * @return JSON格式数据，包括包括servlet的请求Id、实体的GUID。
     */
    @POST
    @Path("{guid}/traits")
    @Consumes(Servlets.JSON_MEDIA_TYPE)
     ...
     *
     * @param guid      实体的GUID
     * @param traitName Trait的名称
     * @return JSON格式数据，包括包括servlet的请求Id、实体的GUID和trait的名称。
     */
    @DELETE
