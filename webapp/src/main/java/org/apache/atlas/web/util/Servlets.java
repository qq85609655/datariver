...
public final class Servlets {
    ...
    public static Response getErrorResponse(Throwable e, Response.Status status) {
        ...
        addErrorCode(e, responseJson);
        try {
            responseJson.put(AtlasClient.STACKTRACE, printStackTrace(e));
        } catch (JSONException e1) {
            LOG.warn("Could not construct error Json response", e1);
        }
        return response;
    }

    private static void addErrorCode(Throwable e, JSONObject responseJson) {
        try {
            if (e instanceof AtlasException) {
                AtlasException exception = (AtlasException) e;
                if (exception.getErrorEnum() != null) {
                    responseJson.put("error_code", exception.getErrorEnum().getCode());
                }
            } else if (e instanceof IllegalArgumentException) {
                responseJson.put("error_code", ErrorEnum.ILLEGAL_ARGUMENT.getCode());
            } else {
                responseJson.put("error_code", ErrorEnum.SYS_ERR.getCode());
            }
        } catch (JSONException e1) {
            LOG.warn("获取error_code异常", e1);
        }
    }
    ...
    public static Response getErrorResponse(String message, Response.Status status) {
        ...
        Object errorEntity = message;
        try {
            errorJson.put(AtlasClient.ERROR, errorEntity);
            errorEntity = errorJson;
        } catch (JSONException jsonE) {
            LOG.warn("Could not construct error Json response", jsonE);
        }
    ...
    public static String escapeJsonString(String inputStr) {
        ...
        if (inputStr == null) {
            inputStr = "XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX";
        }
